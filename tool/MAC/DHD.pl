...
$nbimg='./Data/nbimg_mac';
$fb='./Data/fastboot-mac';
$adb='./Data/adb-mac';
$pb='PD98IMG';
# $get='.\Data\wget.exe';
$ver="DESIRE HD";
...
md5:
system("openssl md5 $pb.zip | grep $md5");
if ($? == 0) {
goto snd;
} else {
print "SOMME MD5 FAUSSE, ON RETOURNE TELECHARGER LE ZIP\n";
unlink glob "$pb.zip";
goto dl;
}
...
enautia:
$url = "http://e-nautia.com/people/tiphergane/disk/HTC%20desire%20multi%20tool/$pb.zip";
goto lnk;
...
print "*" x 80;
$gfv="./Data/gfree_verify";
# envoi des fichiers necessaires au desimlock sur /data/local/
system("$adb push $gfv /data/local/");
system("adb% shell chmod 777 /data/local/gfree_verify");
