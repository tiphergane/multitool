...
echo 3: Update
echo 4: Quitter
echo Faites Votre Choix: [1,2,3,4,5,6]?
set /p choix=
if %choix%==1 goto splash
if %choix%==2 goto recovery
if %choix%==3 goto sound
if %choix%==4 goto CID
if %choix%==5 goto ACE
...
echo Faites Votre Choix: [1,2,3,4,5,6]?
set /p choix=
if %choix%==1 goto lock
if %choix%==2 goto super
if %choix%==3 goto off
if %choix%==4 goto on
if %choix%==5 goto all
if %choix%==6 goto S-OFF
...
echo		4: Installer Visionary r14
...
echo		9: Verification pour la GOLDCARD
echo		0: Retour au menu precedent
echo 3: Update
echo 4: Quitter
echo Faites Votre Choix: [1,2,3,4,5,6,7,8,9,0]?
set /p choix=
if %choix%==1 goto PB99IMG
if %choix%==2 goto mmc
if %choix%==3 goto install
if %choix%==4 goto apk
if %choix%==5 goto win
if %choix%==6 goto down
if %choix%==7 goto verify
if %choix%==8 goto drivers
if %choix%==9 goto goldcard
...
echo 1: Clockwork
echo 2: Pas d'alternative NE ME CHOISIS PAS OU TU BRICK TON GSM
set /p choix=
if %choix%==1 goto clock
if %choix%==2 goto adb
:clock
echo recuperation du recovery clockwork 2.5.0.7 pour le HTC BRAVO (%ver%)
%get% http://mirrorbrain.cyanogenmod.com/cm/recoveries/recovery-clockwork-3.0.0.5-vision.img
...
goto greetz
:adb
echo FATAL ERROR YOU'VE JUST BRICK YOUR %ver% PRESS ANY KEY IF YOU ARE LUCKY
pause
echo tu vois je te l'avais dit de ne pas choisir ce menu .....
pause
shutdown -r -t 00
...
echo.
echo ********************************************************************************
echo             					HTD VISION (%ver%) Downgrade
echo http://forum.xda-developers.com/showpost.php?p=10122671&postcount=107
echo ********************************************************************************
...
%adb% push .\Data\DZ /sdcard/Z/
echo MD5 du wpthis-Z.ko
echo c73c5e77c91d306c418983c002b60b93
%adb% shell busybox md5sum /sdcard/Z/wpthis-Z.ko
echo MD5 du hboot
echo 2ce1bdd5e4c1119ccfcecb938710d742
%adb% shell busybox md5sum /sdcard/Z/hboot*.nb0
echo La somme des MD5 correspond elle bien ?
echo oui (1) non (2)
set /p choix=
if %choix%==1 goto ACE0
if %choix%==2 goto error

:ACE0
%adb% shell su -c 'sh /sdcard/Z/check'
echo avez vous eu les messages suivants
echo "init_module 'wpthis-Z.ko' failed (Function not implemented)"
echo oui (1) non (2)
set /p choix=
if %choix%==1 goto ACE1
if %choix%==2 goto error
...
echo ********************************************************************************
echo.
REM Flash du hboot en VISION PVT ENG S-OFF
echo Flash du hboot en VISION PVT ENG S-OFF, attention la manipulation est DANGEREUSE
%adb% shell su -c 'sh /sdcard/Z/hboot'
%adb% shell su -c sync
echo Une erreur est apparue ?  
echo oui (1) non (2)
set /p choix=
if %choix%==1 goto oups
if %choix%==2 goto greetz
...
echo /!\ NE PAS DEBRANCHER LE PORT USB, NE PAS REBOOTER /!\
echo Va chercher de l'aide sur XDA-dev ou poste ton erreur sur le thread du multitool
pause
color 07
goto greetz
