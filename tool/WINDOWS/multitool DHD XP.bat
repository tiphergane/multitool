...
goto greetz
:adb
echo recuperation du recovery adb pour le HTC BRAVO (%ver%)
...
set gf=.\Data\gfree
set gfv=.\Data\gfree_verify
REM envoi des fichiers necessaires au desimlock sur /data/local/
%adb% push %gf% /data/local/
%adb% push %gfv% /data/local/
%adb% shell chmod 777 /data/local/gfree_verify
%adb% shell chmod 777 /data/local/gfree
...
echo oui (1) non (2)
echo derniere boucle avant les bobos
echo Faites Votre Choix: [1,2]?
set /p choix=
if %choix%==1 goto ACE2
if %choix%==2 goto error
...
set /p choix=
if %choix%==1 goto recovery
if %choix%==2 goto error
if %choix%==2 goto oups
...
REM On renomme le fichier MP3 en MP3.nothanks
